...
import icy.gui.component.RibbonPopup;
...
public class MainRibbon extends MainAdapter implements PluginLoaderListener
{
	 ...
	 *            : unused
	 */
	public static void setPluginButton(final AbstractCommandButton button, final PluginDescriptor plugin, ActionListener action)
	{
		...
		button.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON1) {
					...
					// generate popup menu
					final RibbonPopup menu = new RibbonPopup(plugin);
					IcyDesktopPane pane = Icy.getMainInterface().getMainFrame().getDesktopPane();
					int y = pane.getLocationOnScreen().y - button.getLocationOnScreen().y;
	...
	/**
	 * return true if specified workspace is in conflict with current actives
	 * workspace
	 */
	...
	void removeItem(ItemDefinition item)
	{
		if (item != null)
		{
			...
			item.remove();
			// save system workspace only, we want to preserve plugins
			// organization
			systemWorkspace.save();
		}
	}
	...
	//
	// // add it to the new installed plugins workspace and save it
	// newPluginsBandDef.addItem(plugin.getClassName(),
					...
					if (band == othersPluginsBandDef)
					{
						...
						btn.setPopupCallback(new PopupPanelCallback()
						{
							...
							public JPopupPanel getPopupPanel(JCommandButton commandButton)
							{
								...
								// FIXME : set as heavy weight component for VTK
								// (doesn't work)
	...
	private void buidlTaskBar()
	{
		...
		windowsButton.setPopupCallback(new PopupPanelCallback()
		{
			...
			public JPopupPanel getPopupPanel(JCommandButton commandButton)
			{
				...
				// final IcyCommandMenuButton seButton = new
				// IcyCommandMenuButton("Script Editor",
				// new
