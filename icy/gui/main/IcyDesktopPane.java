...
public class IcyDesktopPane extends JDesktopPane implements ContainerListener,
	...
	public static interface DesktopOverlay extends MouseListener,
	...
	public static class BackgroundDesktopOverlay extends AbstractDesktopOverlay {
		...
		public BackgroundDesktopOverlay() {
			...
			// right now)
			backGround = ResourceUtil.getImage(BACKGROUND_PATH
					+ Integer.toString(Random.nextInt(1)) + ".jpg");
			// load ICY logo
		...
		public void paint(Graphics g, int width, int height) {
			...
			final int textWidth = (int) GraphicsUtil.getStringBounds(g2, text)
					.getWidth();
		...
		for (int i = 0; i < numLine; i++) {
			for (int j = 0; j < numCol; j++, k++) {
				...
				for (int f = 0; f < numFrames; f++) {
					final Point2D.Double center = ComponentUtil
							.getCenter(frames.get(f));
					distances[f] = Point2D.distanceSq(center.x, center.y, x, y);
				}
			}
		}

		final int[] framePos = new HungarianAlgorithm(framesDistances)
				.resolve();
